...
```sh
(開発PC):~/group_x$ git clone https://github.com/matsuolab/roomba_hack.git

(開発PC):~/group_x$ ls
# group_xディレクトリの中に新しいディレクトリができていることを確認

(開発PC):~/group_x$ cd roomba_hack

(開発PC):~/group_x/roomba_hack$ ls
```
...
```sh
# ローカルブランチの一覧を表示
(開発PC):~/group_x/roomba_hack$ git branch
```
{{< /spoiler >}}
...
(開発PC):~/group_x/roomba_hack$ cat BUILD-DOCKER-IMAGE.sh
# ファイルの中身が表示される
...
(開発PC):~/group_x/roomba_hack$ ./BUILD-DOCKER-IMAGE.sh
```
...
```sh
(開発PC):~/group_x/roomba_hack$ ssh roomba_dev2
