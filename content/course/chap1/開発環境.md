...
pwd  # present working directoryのこと
...
- 例えば、PCの中でAというプログラムとBというプログラムを動かしたいとします。そして、これらは他のCというプログラムに依存しているものとします。
  もし、AがCのバージョン1.2で動かすことにのみ対応していて、BはCのバージョン1.3にのみ対応していた場合、Bを動かせるようにCのバージョンを1.3にしてしまうと、Aが実行できなくなってしまいます。  
  このような場合、dockerコンテナを複数作成し（コンテナI、コンテナIIとします）、

    - コンテナIの中にはプログラムAとCのバージョン1.2をインストールして使い、
    - コンテナIIの中にはプログラムBとCのバージョン1.3をインストールして使用する  
